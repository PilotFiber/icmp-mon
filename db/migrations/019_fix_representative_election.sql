-- Migration: 019_fix_representative_election.sql
...
-- Step 1: Add STANDBY to monitoring_state enum
ALTER TYPE monitoring_state ADD VALUE IF NOT EXISTS 'standby';

-- Step 2: Reset all is_representative to false
UPDATE targets SET is_representative = false;

-- Step 3: Elect one representative per subnet
...
-- Step 4: Move other baseline-established customer IPs to STANDBY
...
-- Step 5: Add index for standby targets (failover pool)
...
-- Step 6: Add standby_recheck tier for hourly verification
